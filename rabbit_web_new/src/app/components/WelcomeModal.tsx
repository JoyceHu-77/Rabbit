...
import { ChevronRight, X, ZoomIn, ChevronLeft, Volume2, VolumeX } from 'lucide-react';
...
import welcomeVideo from '../../imports/爱兔会欢迎视频.mp4';
...
export default function WelcomeModal({ open, onClose, onCollapseComplete }: WelcomeModalProps) {
  ...
  const handleStepChange = (step: number) => {
    if (welcomeSteps[step]?.isVideo && videoRef.current) {
  ...
  return (
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="relative mb-6">
              ...
              >
                {step.isVideo ? (
                  <div className="relative w-full h-full">
                    ...
                    <button
                      onClick={toggleMute}
                      className="absolute bottom-3 right-3 p-2 bg-black/60 hover:bg-black/70 rounded-full transition-colors"
                    >
                      {isMuted ? (
              ...
              {/* 图片轮播控制 */}
              {step.images.length > 1 && !step.isVideo && (
                <>
                  {currentImageIndex > 0 && (
                    <button
    ...
    <Dialog open={isImageZoomed} onOpenChange={setIsImageZoomed}>
      <DialogContent className="max-w-4xl w-[95vw] h-[90vh] p-0 overflow-hidden bg-white" showClose={false}>
        ...
        <div className="relative w-full h-full overflow-y-auto">
          <div className="min-h-full flex items-center justify-center p-4">
            <img
              src={step.images[currentImageIndex]}
              alt={step.title}
              className="max-w-full max-h-auto object-contain"
            />
          </div>
        </div>
        ...
        {step.images.length > 1 && (
          <>
            {currentImageIndex > 0 && (
              <button
                onClick={handlePrevImage}
                className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/20 hover:bg-black/30 rounded-full flex items-center justify-center transition-colors"
              >
            ...
            {currentImageIndex < step.images.length - 1 && (
              <button
                onClick={handleNextImage}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/20 hover:bg-black/30 rounded-full flex items-center justify-center transition-colors"
              >
