...
export default function ActivityTab({ isAdmin, onSectionChange }: ActivityTabProps) {
  const [activeView, setActiveView] = useState<'banner' | 'events' | 'shop'>('banner');

  const handleViewChange = (view: 'banner' | 'events' | 'shop') => {
    setActiveView(view);
    onSectionChange?.();
  };
    ...
    <div className="min-h-screen">
      ...
      <div className="sticky top-0 bg-white border-b border-red-100 shadow-sm z-10">
        <div className="flex items-center max-w-2xl mx-auto overflow-x-auto">
          <button
            onClick={() => handleViewChange('banner')}
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {activeView === 'shop' && <CharityShop isAdmin={isAdmin} />}
      </div>
    </div>
  );
}
