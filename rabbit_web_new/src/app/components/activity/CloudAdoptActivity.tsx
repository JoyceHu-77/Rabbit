...
import BadgeDeliveryDialog from './BadgeDeliveryDialog';
...
export default function CloudAdoptActivity() {
  ...
  const [cloudHistory, setCloudHistory] = useState<{ name: string; image: string; date: string }[]>([]);
  ...
  const handleCloudSuccess = (rabbitName: string) => {
    const today = new Date().toISOString().split('T')[0];
    const rabbit = cloudRabbits.find(r => r.name === rabbitName);
    if (rabbit && !cloudHistory.some(h => h.name === rabbitName)) {
      setCloudHistory([...cloudHistory, { name: rabbitName, image: rabbit.image, date: today }]);
    }
  };

  const handleOnlineDelivery = () => {
    setDeliveryMode('online');
    setShowDeliveryDialog(true);
  };

  const handleOfflineDelivery = () => {
