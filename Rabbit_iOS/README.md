...
接口地址配置在 `Rabbit_iOS/Info.plist`：

```xml
<key>RABBIT_API_BASE_URL</key>
<string>http://47.111.225.11:8000</string>
```

如果 `RABBIT_API_BASE_URL` 为空或不是 `http(s)://` 开头，网络层会使用本地回退数据。

## 运行方式

优先打开 workspace，而不是直接打开 xcodeproj：

```bash
open Rabbit_iOS.xcworkspace
```

在 Xcode 中选择 `Rabbit_iOS` scheme，运行到 iOS Simulator 或真机。

也可以使用命令行构建：

```bash
xcodebuild \
  -workspace Rabbit_iOS.xcworkspace \
  -scheme Rabbit_iOS \
  -configuration Debug \
...
- 业务代码优先放入 `Business/<Module>/`。
- 多模块共享 UI 或状态放入 `Common/`。
- 不依赖具体业务的网络、存储、持久化、主题和工具放入 `Base/`。
- App 生命周期、登录门控和根 Tab 装配放入 `App/`。
- 媒体资源保持在 `Resources/`，避免破坏现有 bundle 子目录查找。
- `Info.plist` 保持在 `Rabbit_iOS/Info.plist`，该路径被 Xcode build setting 显式引用。

## 编码约定

- 保持 Swift 类型名不依赖文件路径，同一 target 内按模块目录组织。
- SwiftUI 页面尽量保持视图职责清晰，复杂列表状态下沉到 ViewModel。
- 业务模块避免直接复制通用 UI，优先复用 `Common/Components`。
- 网络请求统一收敛到 `Base/Network/RabbitAPIService*`。
- 本地持久化入口优先使用 `Base/Persistence` 和 `Base/Storage`。

## 测试与验证

当前仓库未发现独立测试 target。提交前建议至少执行：
...
注意：直接构建 `Rabbit_iOS.xcodeproj` 可能找不到 CocoaPods 依赖中的 `Alamofire`，请使用 `Rabbit_iOS.xcworkspace`。
